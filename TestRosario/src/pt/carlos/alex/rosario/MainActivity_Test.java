...
//        this.launchActivity(this.getActivity().getString(pt.carlos.alex.rosario.tests.R.string.package_name), MainActivity_.class, new Bundle());
    ...
    public void testPortraitOK() throws Exception {
        ...
        assertEquals(" Portrait com slidingMenu? ", true, ma.mSlindingMenu != null);

        solo.clickOnImage(0);

        assertEquals(" Behind slidingMenu visivel? ", true, ma.mSlindingMenu.isBehindShowing());
    ...
    public void testLandscapeOK() throws Exception {
        ...
        assertTrue(true);
//        assertEquals(" Landscape sem slidingMenu? ", false, ma.mSlindingMenu != null);
...
//        solo.clickOnActionBarHomeButton();
    ...
    public void testMudaDataCalendario() throws Exception {
        ...
        if (indexDiaSemana == 5) {
            solo.setDatePicker(dPicker, ANO_7, MES_7, DIA_7);     // dia 03-11-2012 Sábado
            solo.clickOnButton("Definir");
            Assert.assertEquals("Mudou para 03-11-2012 Sábado ", true, solo.searchText(DIA_SEMANA[7], 2, false, true));
        } else {
            solo.setDatePicker(dPicker, ANO_5, MES_5, DIA_5);     // dia 01-11-2012 5ª feira
            solo.clickOnButton("Definir");
            Assert.assertEquals("Mudou para 01-11-2012 5ª feira ", true, solo.searchText(DIA_SEMANA[5], 2, false, true));
        }
...
//        solo.clickOnActionBarHomeButton();
        ...
        Assert.assertEquals(" Lista dos mistérios vísivel? ", true, solo.searchText(Misterios.LUMINOSOS[5], 1, false, true));

//        solo.clickInList(2, 0);
        solo.clickOnText(Misterios.LUMINOSOS[1], 1, true);

        Assert.assertEquals(" Mudou para o 2º Mistério? ", true, solo.searchText(MISTERIO_LUMINOSO_2, 1, true, true));
    ...
    public void testMudaOracoes () throws Exception {
        ...
        Assert.assertTrue("1º Mistério Luminoso? ", solo.searchText(MISTERIO_LUMINOSO_1, 1, true, true));
...
//        solo.scrollToSide(Solo.LEFT);
        Assert.assertTrue("Pai Nosso? ", solo.searchText(PAI_NOSSO, 1, true, true));

        for (int i = 0; i < 10; i++) {
            solo.scrollViewToSide(v, Solo.RIGHT);
            Assert.assertTrue(String.format("Avé Maria - %n? ", i), solo.searchText(AVE_MARIA, 1, true, true));
        }
        ...
        solo.scrollViewToSide(v, Solo.RIGHT);
        Assert.assertTrue("Jaculatória? ", solo.searchText(JACULATORIA, 1, true, true));
    ...
    public void testMenuAbout () throws Exception {
        ...
        Assert.assertTrue("Menu About OK? ", solo.searchText(ABOUT, 1, true, true));
    ...
    public void testMenuLicense () throws Exception {
        ...
        Assert.assertTrue("Menu About OK? ", solo.searchText(LICENCA, 1, true, true));
